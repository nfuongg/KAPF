...
export default function AssignmentDetail() {
  ...
  if (!assignment) {
    return (
      <main className="min-h-screen bg-background flex items-center justify-center font-heading text-[#3A2E2A]">
  ...
  return (
    <main className="max-w-[1920px] w-full mx-auto relative bg-background overflow-clip font-heading text-[#3A2E2A]">
      ...
      >
        <div className="rounded-[20px] shadow-[inset_0_0_0_1px_#3A2E2A] p-8 md:p-12 bg-[#F0EBE3]">
      ...
      <section className="mt-[clamp(30px,5vw,80px)] mx-4 md:mx-10 flex flex-col gap-[clamp(24px,4vw,60px)] relative z-10 pb-[clamp(60px,10vw,150px)]">
        {assignment.sections.map((section, si) => (
          ...
          >
            ...
            {/* Content */}
            {section.content && (
              <div className="rounded-[20px] shadow-[inset_0_0_0_1px_#3A2E2A] p-8 bg-[#FFEFEF]">
            ...
            {section.steps && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {section.steps.map((s, i) => (
                  <div
                    key={i}
                    className="flex flex-row items-start gap-4 p-6 rounded-[16px] shadow-[inset_0_0_0_1px_#3A2E2A] bg-[#FFEFEF] hover:bg-[#F0EBE3] transition-all"
                  >
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <p className="text-[clamp(13px,1vw,16px)] font-light leading-[1.6] text-[#3A2E2A]/70">
                        {s.desc}
                      </p>
                    </div>
                  </div>
            ...
            {section.highlights && (
              <div className="flex flex-col gap-3">
                {section.highlights.map((h, i) => (
                  <div
                    key={i}
                    className="flex flex-row items-start gap-4 p-5 rounded-[14px] bg-[#FFEFEF] shadow-[inset_0_0_0_1px_#3A2E2A]"
                  >
        ...
        <motion.div
          ...
          whileInView={{ opacity: 1, y: 0 }}
          viewport={{ once: true }}
          className="flex flex-col sm:flex-row items-center justify-between gap-6 pt-8 border-t border-[#3A2E2A]/20"
        >
          <Link
            to="/#project"
            className="flex flex-row items-center gap-2 py-3 px-6 rounded-[57px] shadow-[inset_0_0_0_1px_#3A2E2A] hover:bg-[#F0EBE3] transition-all font-bold text-[clamp(14px,1.2vw,18px)]"
          >
